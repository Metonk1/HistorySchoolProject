...
public class MainWindow extends Application{
	...
	public void start(Stage primaryStage) throws Exception {
		...
		/*
		 * Student and FAculty Section
		 */
		
		
		//Illegal Arguemtn with multiple clicks good catch
		
		bbp.getAddFacultyBtn().setOnAction(createFaculty ->{
			if(createFaculty.getSource() == bbp.getAddFacultyBtn()){
			//PersonPage pp = new PersonPage();
			Scene scene = new Scene(fp.getFacultyBox());
			stage2.setTitle("Create Faculty");
		...
		bbp.getAddStudentBtn().setOnAction(createStudent ->{ 
			if(createStudent.getSource() == bbp.getAddStudentBtn()){
			Scene studentScene = new Scene(sp.getStudentBox());
			stage2.setTitle("Create Student");
			stage2.setScene(studentScene);
			stage2.show();
			}else{
				stage2.showAndWait();
			}
	...
	/*	add.setOnAction(e -> {
		...
		homeBox.getChildren().addAll(btnPage.getBtnBox(), bbp.getbodyBtnBox(), tbbp.gettextBookBtnBox(), exitBtn);
